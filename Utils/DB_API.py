...
class DB:
    ...
    def create_tables(self):
        ...
        with connect(host=DB_HOST, user=DB_USER, password=DB_PASS,
            ...
            with connection.cursor() as cursor:
                ...
                partner INT,
                FOREIGN KEY (partner) REFERENCES partners(pid) ON DELETE CASCADE,
                bron VARCHAR(255),
    ...
    def get_clients_options(self):
        query = "SELECT id, customer FROM clients;"
        returns = self.execute(query, fetchall=True)
        return returns
    ...
    def get_flights_on(self, month, year):
        ...
        query2 = "SELECT id, return_date FROM clients" \
                ...
                " MONTH(return_date) = %s"
        returns = self.execute(query2, [year, month], fetchall=True)
        return [flights, returns]

    def create_first_user(self):
        query = "INSERT INTO users(login, password) VALUES(%s, %s);"
        returns = self.execute(query, ['login', 'password'], insert=True)
        return returns

    def delete_partner(self, id):
        try:
            query = "DELETE FROM partners WHERE pid = %s;"
            returns = self.execute(query, [id], insert=True)
            return returns
        except Exception as e:
            print(e)
    ...
    def drop_tables(self):
        try:
            query1 = "DROP TABLE clients;"
            query2 = "DROP TABLE partners;"
            for i in [query1, query2]:
                self.execute(i, insert=True)
        except Exception as e:
            print(e)
