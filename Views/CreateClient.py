from PyQt5.QtWidgets import QWidget, QGridLayout, QLineEdit, QLabel, QPushButton, QSizePolicy, QDateEdit,\
    QComboBox
...
class CreateClient(View):
    ...
    def __init__(self, geometry):
        ...
        self.global_widget.setLayout(self.main_layout)

        self.topper = QWidget(parent=self.global_widget)
        self.topper.setObjectName("Darker")
        self.topper.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Preferred)
        self.body = QWidget(parent=self.global_widget)
        self.body.setObjectName("Lighter")
        ...
        self.date = QLabel(text=f"Дата создания: {'' if d.day > 9 else '0'}{d.day}-"
                                f"{'' if d.month > 9 else '0'}{d.month}"
                                f"-{d.year} г.", parent=self.topper)
